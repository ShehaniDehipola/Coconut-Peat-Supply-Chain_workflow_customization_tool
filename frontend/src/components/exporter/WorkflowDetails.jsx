...
const StepCircle = styled.div`
    background-color: ${({active}) => (active ? "#2D3142" : "#ccc")};
    color: white;
    border-radius: 50%;
    width: 30px;
    height: 30px;
...
const WorkflowDetails = ({}) => {
    ...
    const getCurrentStep = () => {
        if (!workflowVersions[0]?.steps?.length) return 0;
        if (!selectedVersion) return 0;

        const hasConfirmed = selectedVersion.status === "draft" || selectedVersion.status === "pending";
        const hasAssignedManufacturer = selectedManufacturer !== "";
        const isSent = selectedVersion.status === "pending";

        if (isSent) return 3;
        if (hasAssignedManufacturer) return 2;
            ...
            <PageContainer>
                ...
                <LeftContainer>
                    <ProgressBarContainer>
                        {["Register Steps", "Confirm Version", "Assign Manufacturer", "Send to Manufacturer"].map((label, index) => (
                            <ProgressStep key={index} active={index <= currentStep}>
                                <StepCircle active={index <= currentStep}>{index + 1}</StepCircle>
                                <StepLabel>{label}</StepLabel>
                                {index < 3 && <StepLine active={index < currentStep}/>}
                            </ProgressStep>
                        ))}
                    </ProgressBarContainer>
                    ...
                    <ButtonsContainer>
                        <Button bgColor="#1565C0" hoverColor="#0D47A1" onClick={handleEditWorkflow}>Edit
                            Version</Button>
                        <Button bgColor="#D32F2F" hoverColor="#B71C1C">Delete Version</Button>
                        <Button bgColor="#4CAF50" hoverColor="#388E3C" onClick={handleConfirmWorkflow}>Confirm
                            Version</Button>
                    </ButtonsContainer>
                </LeftContainer>
