...
export const ManufacturerWorkflowPage = () => {
  ...
  const { user } = useUser(); // user should contain the token
  ...
  const handleStatusChange = async (stepIndex, actionType) => {
    try {
      ...
      if (actionType === "next") {
        ...
        if (!allSubStepsCompleted) {
          toast.error(
            "Please complete all sub-steps before completing the plugin."
          );
          return;
        }
      }
    ...
    await updateWorkflowVersionStatus('completed');
      ...
      if (actionType === "start") {
        // await axios.post(
        //   `http://localhost:5000/api/plugin/grpcFun`,
        //   {
        ...
        toast.success(
          "Plugin execution request sent to the core system successfully."
        );
      }
    } catch (error) {
  ...
  const handleSubStepAction = async (stepIndex, subIndex, actionType) => {
    try {
      ...
      await axios.put(
        `http://localhost:5000/api/workflow/${workflowId}/step/${stepOrder}/substep/${encodeURIComponent(
          sub.name
        )}`,
        payload
      );
  ...
  const confirmSubStepAction = (stepIndex, subIndex, actionType) => {
    ...
    if (
      actionType === "start" &&
      currentPlugin.subSteps[subIndex].status !== "not_started"
    ) {
    ...
    if (
      actionType === "complete" &&
      currentPlugin.subSteps[subIndex].status !== "in_progress"
    ) {
          ...
          <StepWrapper key={index}>
            ...
            <StepContainer>
              ...
              {step.subSteps.map((subStep, subIndex) => (
                <SubStepContainer key={subIndex} status={subStep.status}>
                  <SubStepHeader>
                    ...
                    <div>
                      ...
                      {subStep.status === "not_started" && (
                        <Button
                          onClick={() =>
                            confirmSubStepAction(index, subIndex, "start")
                          }
                        >
                          Start Sub-step
                        </Button>
                      ...
                      {subStep.status === "in_progress" && (
                        <Button
                          onClick={() =>
                            confirmSubStepAction(index, subIndex, "complete")
                          }
                        >
                          Complete Sub-step
                        </Button>
            ...
            <RightColumn>
              {step.executionResult &&
                step.subSteps.some(
                  (sub) =>
                    sub.name === "Grade Husks" && sub.status === "in_progress"
                ) && (
                  <IoTDataContainer>
                    ...
                    <ul>
                      <li>
                        ...
                        {step.executionResult.total}
                      </li>
                      <li>
                        <strong>User Requirement:</strong>{" "}
                        {step.executionResult.userRequirement}
                      </li>
                      <li>
                        <strong>Qualified:</strong>{" "}
                        {step.executionResult.qualified}
                      </li>
                      <li>
                        <strong>Acceptable:</strong>{" "}
                        {step.executionResult.acceptable}
                      </li>
                      <li>
                        <strong>Rejected:</strong>{" "}
                        {step.executionResult.rejected}
                      </li>
                    </ul>
                  </IoTDataContainer>
                )}
